...
namespace Atma
{
    public struct Steering
    {
        ...
        public Vector2 integrate(Vector2 p, Vector2 velocity, float maxForce, float maxVelocity, float mass)
        {
        ...
        public static Steering zero { get { return new Steering() { steering = Vector2.Zero }; } }

        public static Vector2 truncate(Vector2 p, float len)
        {
            var i = len / p.Length();
            i = i < 1.0f ? i : 1.0f;
        ...
        public static Steering seek(float maxVelocity, Vector2 from, Vector2 target)
        {
            return new Steering() { steering = (target - from).ToNormalized() * maxVelocity };
            //steering += (target - from).ToNormalized() * maxVelocity * influence;
        }

        public static Steering arrive(float maxVelocity, Vector2 currentVelocity, Vector2 from, Vector2 target, float radius)
        {
            return arrive(maxVelocity, currentVelocity, from, target, radius, 1f);
        }

        public static Steering arrive(float maxVelocity, Vector2 currentVelocity, Vector2 from, Vector2 target, float radius, float stopModifier)
        {
        ...
        public static Steering flee(float maxVelocity, Vector2 currentVelocity, Vector2 from, Vector2 target)
        {
        ...
        public static Steering wander(float maxVelocity, float maxForce, Vector2 currentVelocity, float currentOrientation, Vector2 from)
        {
            return wander(maxVelocity, maxForce, currentVelocity, currentOrientation, from, 6f, 8f, 1f);
        }

        public static Steering wander(float maxVelocity, float maxForce, Vector2 currentVelocity, float currentOrientation, Vector2 from, float distance, float radius, float angleChange)
        {
            var circleCenter = currentVelocity.ToNormalized() * distance;
            if (circleCenter == Vector2.Zero)
            {
                //currentOrientation = (float)random.NextDouble() * MathHelper.TwoPi;
                circleCenter = new Vector2(MathHelper.Cos(currentOrientation), MathHelper.Sin(currentOrientation)) * distance;
            }

            currentOrientation += (float)random.NextDouble() * angleChange - angleChange * 0.5f;
            var displacement = new Vector2(MathHelper.Cos(currentOrientation), MathHelper.Sin(currentOrientation)) * radius;

            displacement = truncate(circleCenter + displacement, maxForce);
            return seek(maxVelocity, from, from + displacement);
        }

        public static Steering pursue(float maxVelocity, Vector2 from, Vector2 target, Vector2 targetVelocity)
        {
        ...
        public static Steering evade(float maxVelocity, Vector2 currentVelocity, Vector2 from, Vector2 target, Vector2 targetVelocity)
        {
            var distance = (from - target);
            var t = distance.Length / maxVelocity;
    ...
    //        if (circleCenter == Vector2.Zero)
    //        {
    //            _wanderAngle = (float)random.NextDouble() * MathHelper.TwoPi;
    //            circleCenter = new Vector2(MathHelper.Cos(_wanderAngle), MathHelper.Sin(_wanderAngle)) * distance;
    //        }

    //        var displacement = new Vector2(MathHelper.Cos(_wanderAngle), MathHelper.Sin(_wanderAngle)) * radius;
