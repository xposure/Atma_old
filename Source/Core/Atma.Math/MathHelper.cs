...
//     <id value="$Id: MathHelper.cs 2940 2012-01-05 12:25:58Z borrillis $"/>
// </file>
...
namespace Atma
{
	public sealed class MathHelper
	{
        ...
        public static readonly Real TwoPI = (Real)(System.Math.PI * 2f);
        ...
        {
            return new Vector2(Max(lhs.X, rhs.X), Max(lhs.Y, rhs.Y));
        }
        ...
        {
            return new Vector2(Min(lhs.X, rhs.X), Min(lhs.Y, rhs.Y));
        }
        ...
        /// <summary>
        /// Returns the smallest integer less than or equal to the specified value.
        /// </summary>
        /// <param name="number"></param>
        /// <returns></returns>
        public static Real Floor(Real number)
        {
            return (Real)System.Math.Floor(number);
        }
		                                          ...
		                                          Vector3 b, Vector3 c, Vector3 normal, bool positiveSide, bool negativeSide )
		{
			...
			int i0, i1;
			{
				Real n0 = MathHelper.Abs( normal[ 0 ] );
				Real n1 = MathHelper.Abs( normal[ 1 ] );
				Real n2 = MathHelper.Abs( normal[ 2 ] );
		...
		/// </returns>
		/// Submitted by: romout
		public static Intersection Intersects( AxisAlignedBox3 boxA, AxisAlignedBox3 boxB )
		{
		...
		public static IntersectResult Intersects( Ray3 ray, Sphere sphere )
		{
		...
		public static IntersectResult Intersects( Ray3 ray, Sphere sphere, bool discardInside )
		{
			Contract.RequiresNotNull( ray, "ray" );
			...
			else
			{
				...
				Real t = ( -b - MathHelper.Sqrt( d ) ) / ( 2 * a );

				if( t < 0 )
				{
					t = ( -b + MathHelper.Sqrt( d ) ) / ( 2 * a );
				}
		...
		/// <param name="plane"></param>
		/// <returns>Struct that contains a bool (hit?) and distance.</returns>
		public static IntersectResult Intersects( Ray3 ray, Plane plane )
		{
			Contract.RequiresNotNull( ray, "ray" );

			Real denom = plane.Normal.Dot( ray.Direction );

			if( MathHelper.Abs( denom ) < Real.Epsilon )
			{
				// Parellel
				return new IntersectResult( false, 0 );
			}
		...
		public static bool Intersects( Sphere sphere, Plane plane )
		{
			...
			return MathHelper.Abs( plane.Normal.Dot( sphere.Center ) ) <= sphere.Radius;
		}
		...
		/// <param name="d2"></param>
		/// <returns></returns>
		public static Tuple<bool, Real, Real> Intersect( Ray3 ray, AxisAlignedBox3 box )
		{
			if( box.IsNull )
			{
		...
		/// <returns>Struct that contains a bool (hit?) and distance.</returns>
		public static IntersectResult Intersects( Ray3 ray, PlaneBoundedVolume volume )
		{
			Contract.RequiresNotNull( ray, "ray" );
        ...
        //    int i0, i1;
        //    {
        //        float n0 = Math.MathHelper.Abs(normal[0]);
        //        float n1 = Math.MathHelper.Abs(normal[1]);
        //        float n2 = Math.MathHelper.Abs(normal[2]);
        ...
        public static IntersectResult Intersects(Ray2 ray, Circle circle)
        {
        ...
        /// <param name="discardInside"></param>
        /// <returns>Struct that contains a bool (hit?) and distance.</returns>
        public static IntersectResult Intersects(Ray2 ray, Circle circle, bool discardInside)
        {
            //Contract.RequiresNotNull(ray, "ray");
            //Contract.RequiresNotNull(sphere, "sphere");
            ...
            //Adjust ray origin relative to sphere center
            Vector2 rayOrig = ray.Origin - circle.Center;
            ...
            else
            {
                ...
                // But we only want the closest one, so that's ok, just use the 
                // '-' version of the solver
                float t = (-b - MathHelper.Sqrt(d)) / (2 * a);
        ...
        /// <param name="box"></param>
        /// <returns>A Pair object containing whether the intersection occurred, and the distance between the 2 objects.</returns>
        public static IntersectResult Intersects(Ray2 ray, AxisAlignedBox box)
        {
            //Contract.RequiresNotNull(ray, "ray");
            //Contract.RequiresNotNull(box, "box");
        ...
        public static Vector2 ScaleToSize(Vector2 size, Vector2 targetSize)
        {
        ...
        public static Real WrapAngle(Real angle)
        {
		...
		{
			Real nom = System.Math.Exp( -MathHelper.Sqr( x - offset ) / ( 2 * MathHelper.Sqr( scale ) ) );
			Real denom = scale * MathHelper.Sqrt( 2 * MathHelper.PI );
            ...
            where T : IRadixKey
        {
            ...
            // number of groups 
            int groups = (int)MathHelper.Ceiling((double)b / (double)r);
