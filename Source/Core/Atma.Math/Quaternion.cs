...
//     <id value="$Id: Quaternion.cs 2940 2012-01-05 12:25:58Z borrillis $"/>
// </file>
...
namespace Atma
{
	...
	public struct Quaternion
	{
		...
		public Real PitchInDegrees { get { return MathHelper.RadiansToDegrees( Pitch ); } set { Pitch = MathHelper.DegreesToRadians( value ); } }
		public Real YawInDegrees { get { return MathHelper.RadiansToDegrees( Yaw ); } set { Yaw = MathHelper.DegreesToRadians( value ); } }
		public Real RollInDegrees { get { return MathHelper.RadiansToDegrees( Roll ); } set { Roll = MathHelper.DegreesToRadians( value ); } }
		...
		public Real Pitch
		{
			...
			{
				Real test = x * y + z * w;
				if( MathHelper.Abs( test ) > 0.499f ) // singularity at north and south pole
				{
					return 0f;
				}
				return (Real)MathHelper.ATan2( 2 * x * w - 2 * y * z, 1 - 2 * x * x - 2 * z * z );
			}
		}
		...
		public Real Yaw
		{
			...
			get
			{
				...
				if( MathHelper.Abs( test ) > 0.499f ) // singularity at north and south pole
				{
					return MathHelper.Sign( test ) * 2 * MathHelper.ATan2( x, w );
				}
		...
		public Real Roll
		{
			...
			get
			{
				...
				if( MathHelper.Abs( test ) > 0.499f ) // singularity at north and south pole
				{
					return MathHelper.Sign( test ) * MathHelper.PI / 2;
				}
				return (Real)MathHelper.ASin( 2 * test );
			}
		}

        public Real X { get { return x; } set { x = value; } }
        public Real Y { get { return y; } set { y = value; } }
        public Real Z { get { return z; } set { z = value; } }
        public Real W { get { return w; } set { w = value; } }
		...
		public static Quaternion Slerp( Real time, Quaternion quatA, Quaternion quatB, bool useShortestPath )
		{
			...
			Real angle = (Real)MathHelper.ACos( cos );

			if( MathHelper.Abs( angle ) < EPSILON )
			{
				return quatA;
			}

			Real sin = MathHelper.Sin( angle );
			Real inverseSin = 1.0f / sin;
			Real coeff0 = MathHelper.Sin( ( 1.0f - time ) * angle ) * inverseSin;
			Real coeff1 = MathHelper.Sin( time * angle ) * inverseSin;
		...
		public static Quaternion FromAngleAxis( Real angle, Vector3 axis )
		{
			...
			Real sin = MathHelper.Sin( halfAngle );
		...
		{
			Real factor = 1.0f / MathHelper.Sqrt( this.Norm );
		...
		public void ToAngleAxis( ref Real angle, ref Vector3 axis )
		{
			...
			if( sqrLength > 0.0f )
			{
				angle = 2.0f * (Real)MathHelper.ACos( w );
				Real invLength = MathHelper.InvSqrt( sqrLength );
				axis.X = x * invLength;
				axis.Y = y * invLength;
				axis.Z = z * invLength;
			}
		...
		public Quaternion Log()
		{
			...
			{
				Real angle = (Real)MathHelper.ACos( w );
				Real sin = MathHelper.Sin( angle );

				if( MathHelper.Abs( sin ) >= EPSILON )
				{
		...
		public Quaternion Exp()
		{
			...
			Real angle = MathHelper.Sqrt( x * x + y * y + z * z );
			Real sin = MathHelper.Sin( angle );
		...
		{
			Real fCos = Dot( rhs );
			Real angle = (Real)MathHelper.ACos( fCos );

			return MathHelper.Abs( angle ) <= tolerance;
		}
